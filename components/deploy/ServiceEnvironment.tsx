...
export default function ServiceEnvironment({ service, onUpdateService }: ServiceEnvironmentProps) {
  ...
  const handleUpdateEnvironment = (index: number, field: "key" | "value", value: string) => {
  ...
  return (
    <Card className="subnet-card">
      ...
      >
        ...
        <div className="flex items-center gap-2">
          <span className="text-sm text-default-500">{environmentVars.length} variables</span>
        ...
        <CardBody className="space-y-4">
          {environmentVars.length === 0 ? (
            <div className="text-center py-8 text-default-500">
              ...
              <p className="text-xs text-default-400 mt-1">Click "Add Variable" to get started</p>
            </div>
          ) : (
            <div className="space-y-3">
              {environmentVars.map((env: any, index: number) => (
                <div key={index} className="flex items-center gap-2 p-3 bg-default-50 rounded-lg border border-default-200">
                  <div className="flex-1">
                    <label className="text-sm text-default-600 mb-1 block">Key</label>
                    <Input
                      placeholder="DATABASE_URL"
                      value={env.key}
                      onChange={(e) => handleUpdateEnvironment(index, "key", e.target.value)}
                      size="sm"
                    />
                  </div>
                  <div className="flex-1">
                    <label className="text-sm text-default-600 mb-1 block">Value</label>
                    <Input
                      placeholder="postgresql://user:pass@localhost:5432/db"
                      value={env.value}
                      onChange={(e) => handleUpdateEnvironment(index, "value", e.target.value)}
                      size="sm"
                    />
                  </div>
