...
import { CheckCircle, Cpu, MemoryStick, HardDrive, Gpu, DollarSign } from "lucide-react";
        ...
        <CardBody className="space-y-4">
          <div className="grid md:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <span className="font-medium">{description || "No description"}</span>
            </div>
            <div>
            ...
            <div>
              ...
              <span className="font-medium text-success">{estimatedPrice} SCU/hour</span>
            </div>
          </div>
        </CardBody>
      </Card>
          ...
          {(() => {
            ...
            return gpuReqs.totalUnits > 0 ? (
              <div className="mt-4 p-4 bg-primary/5 rounded-lg border border-primary/20">
                <h4 className="font-semibold text-primary mb-3">GPU Requirements</h4>
        ...
        <CardBody className="space-y-4">
          <div className="grid md:grid-cols-2 gap-4">
            {services.map((service, index) => (
              <div key={index} className="border border-default-200 rounded-lg p-4 bg-gradient-to-br from-default-50 to-default-100/50 hover:shadow-md transition-shadow">
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div className="bg-white/60 p-3 rounded-lg border border-default-200">
                    <div className="flex items-center gap-2 mb-1">
                      ...
                      <span className="text-xs font-medium text-default-600">CPU</span>
                    </div>
                  ...
                  <div className="bg-white/60 p-3 rounded-lg border border-default-200">
                    <div className="flex items-center gap-2 mb-1">
                      ...
                      <span className="text-xs font-medium text-default-600">Memory</span>
                    </div>
                    <div className="text-sm font-semibold text-success">
                      {(service.resources.memory as any)?.size || (service.resources.memory as any)?.units + "Gi" || "0Gi"}
                    </div>
                  </div>
                  ...
                  <div className="bg-white/60 p-3 rounded-lg border border-default-200">
                    <div className="flex items-center gap-2 mb-1">
                      ...
                      <span className="text-xs font-medium text-default-600">Storage</span>
                    </div>
                    <div className="text-sm font-semibold text-warning">
                      {Array.isArray(service.resources.storage) && service.resources.storage.length > 0 
                        ? (service.resources.storage[0] as any)?.size 
                        : (service.resources.storage as any)?.size || (service.resources.storage as any)?.units + "Gi" || "0Gi"}
                    </div>
                  </div>
                  ...
                  <div className="bg-white/60 p-3 rounded-lg border border-default-200">
                    <div className="flex items-center gap-2 mb-1">
                      ...
                      <span className="text-xs font-medium text-default-600">GPU</span>
                    </div>
                ...
                <div className="bg-white/40 p-3 rounded-lg border border-default-200">
                  <div className="grid grid-cols-2 gap-3 text-xs">
                    <div>
                      ...
                      <div className="font-medium text-default-700 truncate" title={service.image}>
                        {service.image}
                      </div>
                    </div>
                    <div>
                      <span className="text-default-500">Replicas:</span>
                      <div className="font-medium text-default-700">{service.replicas || 1}</div>
                    </div>
                  </div>
                </div>
              </div>
              ...
              {(service.command || service.args || (service as any).registryUrl || 
                (service.volumes && service.volumes.length > 0) || 
                ((service as any).expose && (service as any).expose.length > 0) || 
                ((service as any).ports && (service as any).ports.length > 0) ||
                (((service as any).env && (service as any).env.length > 0) || ((service as any).environment && (service as any).environment.length > 0))) && (
                <div className="mt-3 space-y-3">
                  {/* Command & Args */}
                  {(service.command || service.args) && (
                    <div className="bg-white/30 p-3 rounded-lg border border-default-200">
                      <div className="text-xs font-medium text-default-600 mb-2">Execution</div>
                      {service.command && (
                        <div className="text-xs">
                          ...
                          <div className="font-mono text-default-700 bg-default-100 px-2 py-1 rounded mt-1">
                            {Array.isArray(service.command) ? service.command.join(' ') : service.command}
                          </div>
                        </div>
                      )}
                      {service.args && (
                        <div className="text-xs mt-2">
                          ...
                          <div className="font-mono text-default-700 bg-default-100 px-2 py-1 rounded mt-1">
                            {Array.isArray(service.args) ? service.args.join(' ') : service.args}
                          </div>
                        </div>
                      )}
                    </div>
                  ...
                  {service.volumes && service.volumes.length > 0 && (
                    <div className="bg-white/30 p-3 rounded-lg border border-default-200">
                      <div className="text-xs font-medium text-default-600 mb-2">Volumes ({service.volumes.length})</div>
                      <div className="space-y-2">
                        {service.volumes.map((volume: any, volIndex: number) => (
                          <div key={volIndex} className="bg-white/60 p-2 rounded text-xs">
                            <div className="flex justify-between items-center">
                              <span className="font-medium">{volume.name}</span>
                              <span className="text-default-500">{volume.size}{volume.sizeUnit}</span>
                            </div>
                            <div className="text-default-600 mt-1">
                              {volume.type} → {volume.mount} {volume.readOnly ? "(read-only)" : ""}
                            </div>
                          </div>
                        ))}
                      </div>
                  ...
                  {/* Ports */}
                  {(((service as any).expose && (service as any).expose.length > 0) || ((service as any).ports && (service as any).ports.length > 0)) && (
                    <div className="bg-white/30 p-3 rounded-lg border border-default-200">
                      <div className="text-xs font-medium text-default-600 mb-2">Ports ({(service as any).ports?.length || (service as any).expose?.length || 0})</div>
                      <div className="space-y-2">
                        {((service as any).ports || (service as any).expose || []).map((port: any, portIndex: number) => (
                          <div key={portIndex} className="bg-white/60 p-2 rounded text-xs">
                            <div className="flex justify-between items-center">
                              <span className="font-medium">{port.port}→{port.as || port.port}</span>
                              <span className="text-default-500">({port.protocol})</span>
                            </div>
                            {(port.domains && port.domains.length > 0) || (port.acceptDomains && port.acceptDomains.length > 0) ? (
                              <div className="text-default-600 mt-1">
                                Domains: <span className="font-medium">{(port.domains || port.acceptDomains || []).join(', ')}</span>
                              </div>
                            ) : null}
                  ...
                  {(((service as any).env && (service as any).env.length > 0) || ((service as any).environment && (service as any).environment.length > 0)) && (
                    <div className="bg-white/30 p-3 rounded-lg border border-default-200">
                      <div className="text-xs font-medium text-default-600 mb-2">Environment ({(service as any).environment?.length || (service as any).env?.length || 0})</div>
                      <div className="flex flex-wrap gap-1">
                        {((service as any).environment || (service as any).env || []).map((env: any, envIndex: number) => (
                          <Chip key={envIndex} color="default" size="sm" variant="flat">
                            {env.key}={env.value}
                          </Chip>
                        ))}
                      </div>
