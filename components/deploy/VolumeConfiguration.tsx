...
export default function VolumeConfiguration({ service, onUpdateService }: VolumeConfigurationProps) {
  ...
  const handleRemoveVolume = (index: number) => {
    const currentVolumes = service.volumes || [];
    const newVolumes = currentVolumes.filter((_: any, i: number) => i !== index);
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-medium text-default-700">Volume Configuration</span>
        </div>
        <Button
          size="sm"
          color="primary"
          ...
          {service.volumes.map((volume: any, index: number) => (
            <div key={index} className="p-4 bg-default-50 rounded-lg border border-default-200">
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                <div>
                  <label className="text-sm text-default-600 mb-1 block">Volume Name</label>
                  <Input
                ...
                <div>
                  <label className="text-sm text-default-600 mb-1 block">Type</label>
                  <Select
                    selectedKeys={[volume.type && VOLUME_TYPES.find(t => t.id === volume.type) ? volume.type : "ssd"]}
              ...
              <div className="flex items-end gap-2">
                <div className="flex-1">
                  <label className="text-sm text-default-600 mb-1 block">Size</label>
                  <Input
                    placeholder="1"
                ...
                <div className="w-24">
                  <label className="text-sm text-default-600 mb-1 block">Unit</label>
                  <Select
              ...
              <div>
                <label className="text-sm text-default-600 mb-1 block">Mount Path</label>
                <Input
                  placeholder="/data"
                  value={volume.mountPath}
                  onChange={(e) => handleUpdateVolume(index, "mountPath", e.target.value)}
                  size="sm"
                />
              </div>
